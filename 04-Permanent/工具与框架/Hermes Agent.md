...
> [!quote] 来源
> 本节内容提炼自 [[Hermes Agent 满配实战（3万字）：从 Memory、Skill 到多 Agent 协同（上手指南）]]，与上文架构源码分析互补——架构篇回答"Hermes 是怎么设计的"，实战篇回答"Hermes 怎么从裸装变成一个真正能长期运行的个人 Agent 系统"。

### 满配的核心定义

> **满配不是"装满"，而是围绕真实工作模式，把输入、记忆、技能、工具、自动化、可视化、Token 成本和多 Agent 协同组合成一个可长期维护、可持续进化的个人 Agent 系统。**
...
| 层级 | 模块 | 解决的问题 | 推荐程度 |
|------|------|-----------|---------|
| L1 | 安装与模型 Provider | 让 Hermes 稳定跑起来 | 必配 |
| L2 | 输入系统（SOUL / AGENTS / CLAUDE.md） | 让 Hermes 理解你和当前项目 | 必配 |
| L3 | Memory 长期记忆 | 跨会话记住偏好、目标、经验和环境 | 必配 |
| L4 | Skills 技能系统 | 把稳定流程沉淀成可复用能力 | 必配 |
| L5 | Tools / Toolsets | 控制 Hermes 能调用哪些本地能力 | 必配 |
| L6 | MCP 外部工具连接 | 接 GitHub、文件系统、搜索、数据库等 | 进阶必配 |
| L7 | Gateway 消息入口 | 让 Hermes 出现在飞书 / Telegram / Discord | 按需配置 |
...
```
SOUL.md      → Agent 应该怎么工作（立场、职责、边界、风格）
USER.md      → 我是谁、偏好、期望
MEMORY.md    → 我在做什么项目、环境事实、踩坑经验
AGENTS.md    → 当前目录的规则和说明
SKILL.md     → 可复用任务流程
当前对话      → 临时任务和上下文
```

> 信息分层原则：USER.md 记录"我是谁"，MEMORY.md 记录"我在做什么"，Skill 记录"我怎么做事"，Session 记录"这次发生了什么"，文件系统保存"大量原始资料"。

#### SOUL.md：写工作协议，不是写人设卡

SOUL.md 应该回答"你应该怎么和我协作"，而非"你是什么性格"。核心维度：

- **立场**：告诉 Agent 可以主动、可以反对，不需要事事请示
- **自主性边界**：明确哪些事可以自己做（低风险推进）、哪些必须确认（公开发布、购买、发消息、不可逆修改）
- **反对与纠偏**：让 Agent 知道何时该说"不"，每个反对意见需附带数据/例子/推理或更好替代方案
- **自我改进**：把重复出现的流程沉淀成检查清单、模板、脚本或 Skill

> **好的 SOUL.md 不是让 Agent 听起来更像人，而是让它做事更像靠谱的搭档。**

#### USER.md + MEMORY.md 模板要点

| 文件 | 定位 | 容量 | 适合写什么 |
|------|------|------|-----------|
| USER.md | 长期画像 | ~1375 字符 | 身份、偏好、沟通风格、期望 |
| MEMORY.md | Agent 工作笔记 | ~2200 字符 | 项目、环境、踩坑、工作流、决策原则 |

**记忆写入原则**——适合写入：长期项目背景、稳定偏好、环境事实、反复踩坑、活跃目标、已验证的决策原则。不适合写入：人设口号、临时任务、大段资料、API Key/密码、过期信息和临时情绪。

> **Memory 不是越完整越好，而是越高频、稳定、可复用、能改变回答质量越好。**

#### 记忆初始化：让 Hermes 通过访谈帮你写

直接让 Hermes 通过访谈帮你生成 USER.md 和 MEMORY.md。访谈覆盖 6 个方面：用户画像、沟通偏好、当前项目、工作流、决策原则、工具环境。访谈结束后 Hermes 自动生成两份内容。

> Memory 应该像工作笔记一样定期清理和更新。Agent 反复犯同一个错误时，说明该写进 Memory；发现 Memory 里有过期项目时，说明该清理了。

#### AGENTS.md：让 Hermes 理解当前目录

如果你同时在多个项目间切换，每个项目放一个 AGENTS.md 比把所有项目信息塞进 MEMORY.md 更有效——因为 MEMORY.md 每次都会加载，而 AGENTS.md 只在进入那个目录时才加载。

#### 好的日常输入模板

好的输入不是说长话，而是把这四件事说清楚：**背景 → 目标 → 约束 → 输出格式**。如果你发现自己反复用同一种方式描述任务，说明这个模式应该沉淀成 Skill。
...
> **Skill 是程序化记忆：当你反复用同一种方式完成任务时，把这套流程写成文档，让 Hermes 以后自动复用。**

| 概念 | 解决什么 | 例子 |
|------|---------|------|
| Memory | 记住"你是谁"和"你在做什么" | 偏好、项目背景 |
| Skill | 记住"你怎么做事" | 分析工具是否值得装的固定流程 |
| MCP Tool | 连接"你能用什么外部工具" | GitHub 搜索 |
...
| Skill | 定位 | 解决什么 |
|-------|------|---------|
| **gstack** | 高强度工作流与技能栈 | Agent 如何高质量做事 |
| **gbrain** | Agent 的共享外部大脑 | Agent 如何长期记住并召回上下文 |
| **hermes-webui** | 轻量化可视化控制台 | 降低使用门槛 |
| **self-evolution** | 自进化核心模块 | 让 AI 持续自主迭代 |
| **awesome-hermes** | 全生态项目合集导航 | 快速查找工具、教程、最佳实践 |

> **常驻 Skill 不超过 5 个。**超过 5 个说明你在用 Skill 数量代替 Skill 质量。每周清理问 3 个问题：过去 7 天实际用了哪些？有没有两个 Skill 做的事重叠？有没有输出质量下降了？
...
- 工具系统的核心不是"全开"，而是"按需开放、分级审批"

安全审批四级：安全（无需审批）→ 低风险（首次审批可记住）→ 中风险（每次审批）→ 高风险（强制审批不可跳过）。

---

### MCP 实战要点

> **MCP 让 Agent 接入外部服务像插 USB-C 一样简单。**

#### GitHub 接入实操关键

- Token 用 Fine-grained tokens 限定只访问特定仓库，每 90 天轮换
- 不要把 Token 写进 config.yaml 明文，用 `${GITHUB_TOKEN}` 引用环境变量
- `allowed_tools` 白名单模式：只列出允许的工具，其余屏蔽
- 读写分离：同一个服务配两个实例（只读版日常用，读写版 `enabled: false` 按需开启）
...
|------|------|
| 是否补足内置工具做不到的事？ | 是才装 |
| 是否需要高权限（删除、发送、支付）？ | 高权限谨慎，用 allowed_tools 收窄 |
| 是否会暴露隐私数据？ | 会就先不上 |
| 是否能限定目录或只读模式？ | 优先选择有限定的 |
| 是否会显著增加 Token 开销？ | 会就用 allowed_tools 过滤 |
| 是否只在某个场景需要？ | 只装到对应 Profile |

> **MCP 的关键不是多，而是窄。只暴露当前任务真正需要的工具。**

---

### Gateway 实战要点（飞书接入）

推荐流程：`hermes gateway setup` → 选择 Feishu/Lark → 扫码创建 Bot → `hermes gateway` 前台启动测试 → `hermes gateway install` 后台服务。

关键配置：
- **连接模式优先选 WebSocket**——不需要公网服务器，不需要 ngrok
- **必须配置 `FEISHU_ALLOWED_USERS`**——避免陌生人使用你的 Bot
- **群聊里 @ Bot 使用**——避免对无关消息频繁触发
...
> Cron 让 Hermes 从被动工具变成主动系统：在固定时间自动执行任务。
...
- **可自动**：搜集公开信息、生成报告保存本地、发送通知到自己、做只读检查
- **必须人工**：安装/卸载工具、修改 Memory/配置、提交代码/部署、发送对外消息、删除文件

> **Cron 只负责"发现和建议"，不负责"擅自改变系统"。**
...
| 组成部分 | Token 风险 |
|---------|-----------|
| 系统提示 + SOUL.md | 固定，通常不大 |
| MEMORY + USER.md | 中等，随积累变大 |
| 项目规则文件 | 中等 |
| 已启用 Skills | 看数量和长度 |
| MCP/Tool schema | 工具多时变成大头 |
| 会话历史 | 随时间增长 |
| 工具输出 | 最不可控，最容易暴涨 |

> **越大不等于越好。**真正重要的是让 Agent 在正确时间看到正确的信息。长上下文会导致更慢、更贵、更不稳、更容易丢重点（Lost in the Middle）。
...
compression:
  enabled: true
  threshold: 0.50            # 上下文窗口 50% 时触发
  target_ratio: 0.20         # 压缩后保留 threshold 的 20% 作为近期上下文
  protect_last_n: 20         # 至少保护最近 20 条消息
  hygiene_hard_message_limit: 400  # 网关安全阈值
```

> 压缩模型的上下文窗口最好不要小于主模型，否则压缩可能失败导致"突然失忆"。

#### 日常精简 5 条

1. **Profile 隔离**——不要把所有工具塞进 default
2. **MCP/Tool 按需启用**——日常聊天只保留最常用工具
3. **控制文件读取范围**——明确指定行数和搜索范围，不要让 Agent 读完整日志
...
| 命令 | 用途 | 什么时候用 |
|------|------|-----------|
| `hermes status` | 快速概览 | 每天看一眼 |
| `hermes doctor` | 深度诊断 | 感觉哪里不对 |
| `hermes logs` | 日志追踪 | 排障定位 |

社区可视化工具：
- **hermes-webui**：和 CLI 1:1 对齐的 Web 界面（推荐想降低门槛的人）
- **hermes-workspace**：偏向"控制中心"，特色是 Conductor（并行 Agent 编排）和 Dashboard（Token 趋势图）

> **终端命令是日常标配，可视化工具是锦上添花。**先把 status/doctor/logs 用熟再考虑加 Web UI。

---

### Profiles 多实例隔离

> **Profile = 一套独立的 Hermes 工作环境。**每个 Profile 有自己的 config、API Key、SOUL.md、memory、sessions、skills、gateway 状态、cron 任务。

#### 重要提醒

Profile 隔离的是 Hermes 内部状态，不是操作系统权限。不同 Profile 里的 Agent 仍然可能访问当前系统用户有权限的文件。SOUL.md 是行为约束，不是系统权限控制。

#### 创建方式对比

| 方式 | 效果 | 适合场景 |
|------|------|---------|
| `create NAME` | 空白 Profile | 从零配置 |
| `create NAME --clone` | 克隆基础配置（config/.env/SOUL.md） | 新建干净 Agent，沿用模型 |
| `create NAME --clone-all` | 尽量完整复制 | 备份或完整 fork |

> 日常新建 Agent 优先用 `--clone`，不要一上来就用 `--clone-all`——会把旧状态也带过去。
...
|---------|------|
| `default` | 日常聊天、临时任务 |
| `radar-agent` | AI 工具雷达、信息报告 |
| `writing-agent` | 公众号写作、选题、素材 |

原则：**任务边界越清晰，越适合拆成独立 Profile。但不要为了"看起来很 Agent 团队"而拆太多。**

---

### 24h Agent 团队架构

> 多 Agent 不是让几个 AI 自动变成一家公司，更现实的理解是：**多个独立 Profile + 明确分工 + 共享目录 + 定时任务 + 人工确认**。

一个可落地的 24h Agent 团队不是每个 Agent 24 小时烧 Token，而是：**该常驻的常驻，该定时的定时，该手动的手动。**

#### 最小团队架构

```
Commander（Gateway 常驻，统一入口）
├── Radar Agent（每天 Cron 定时搜集信息）
├── Writing Agent（需要时手动启动）
└── Cleaner Agent（每周 Cron 定时清理）
```
...
```
～/hermes-lab/shared/
├── reports/           # 各 Agent 输出的报告
├── decisions/         # 人工确认过的决策
└── skill-candidates/  # 待沉淀成 Skill 的候选流程
```

不同 Agent 之间先通过文件交换信息，不要一上来追求复杂调度。Commander 不天然拥有"远程调用另一个 Profile"的能力——需要额外封装或通过共享目录协作。

#### 核心原则

1. **Profile 负责隔离**
2. **Gateway 负责入口**——MVP 只让 Commander 接 Gateway
3. **Cron 负责定时**
4. **共享目录负责协作**
5. **人工确认负责安全**
6. **先手动，再自动**
...
| 阶段 | 目标 | 关键动作 |
|------|------|---------|
| 0 | 跑通基础环境 | `hermes setup` → `hermes model` → `hermes doctor` → 能正常聊天 |
| 1 | 配置输入系统 | SOUL.md + AGENTS.md + 实验目录 + 任务输入模板 |
| 2 | 配置 Memory 和 Skills | 原生 Memory + 少量 Skill + 写 1 个自己的 Skill |
| 3 | 配置 MCP 和工具边界 | 接 1-2 个有用的外部工具，不全量打开 |
| 4 | 配置 Gateway 和 Cron | Hermes 主动报告，只做只读任务 |
| 5 | 配置 Profiles | 不同场景隔离 |
| 6 | 尝试 24h Agent 团队 | Commander + Radar + Cleaner，共享目录协作 |
...
核心流程：创建 `radar-agent` Profile → 配 SOUL.md → 手动跑一次验证 → 可选：本地状态快照脚本 + GitHub CLI 搜集 + 自定义 Skill → 接 Cron 每天自动跑 → 可选接 Gateway 推送到飞书。

Skill 示例（`ai-tool-update-advisor`）：8 个判断维度——是否增强核心能力、是否减少重复劳动、是否可沉淀为长期能力、是否与现有系统重复、是否需要过高权限、是否有供应链风险、是否只是短期噱头、是否让系统更复杂但收益不明显。

> **以前我看到新工具就想装，现在我让 Hermes 先帮我判断：它到底是新能力，还是新噪音。**

#### 实战二：头脑风暴聊天室 Agent

核心流程：创建 `brainstorm-room` Profile → 配 SOUL.md（4 轮讨论规则）→ 创建角色文件 → 手动跑一次验证 → 记忆策略（只把稳定结论写进 Memory，讨论过程保存在 session/output）。

角色设计：系统架构师（关注输入/输出/模块/边界）、产品怀疑者（检查用户价值）、投资人视角（看壁垒和变现）、哲学观察者（找底层假设）、写作编辑（看信息密度）、反方辩手（找最致命反例）。
...
- MCP（先接 GitHub 或 Filesystem）+ Cron（每日/每周只读任务）
- Gateway（Telegram 或飞书）+ Profiles（至少 3 个场景隔离）+ Skill 清理（每周一次）

#### 进阶再上

- Langfuse/Tokscale 精细监控 / Voice Mode / 多 Agent 团队 / 社区技能包 / 第三方上下文引擎 / Browser/Database MCP

#### 暂不建议新手碰

- 全量 MCP / 大量社区 Skill / 自动安装删除工具 / 自动提交代码部署 / 公司数据接 Gateway / 不明来源一键脚本

---

> **满配之后，Hermes 变成了什么？不是"装了 100 个插件"，而是有了更好的输入、更稳的记忆、更清晰的技能、更受控的工具、更主动的自动化、更低的 Token 成本、更可观察的运行状态。最终它不只是一个 Agent，而是逐渐"装下你自己"的系统。**