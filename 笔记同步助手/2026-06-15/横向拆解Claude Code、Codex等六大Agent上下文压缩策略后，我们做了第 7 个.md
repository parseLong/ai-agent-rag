...
author: 腾讯程序员
source: 微信公众号
url: https://mp.weixin.qq.com/s/BQwyvE2qIfguwKk63F3LZw
saved: 2026-06-15 15:36:50
tags:
  - 笔记同步助手
  - 已整理
id: 12ff853d-fd10-46e9-9841-37614cc5cea7
...
![[笔记同步助手/images/65012b7c948ae18452bf7c5db6882ad1_MD5.png]]
...
1.  Budget Reduction——调整工具输出的截断预算
    
2.  Snip——截短老的工具输出，留"做过什么"的摘要行
    
3.  Microcompact——对工具输出内容做局部内联压缩
    
4.  Context Collapse——对更久远的历史做粒度更细的折叠
    
5.  Auto-Compact——兜底，调 LLM 生成结构化摘要
...
-   cached\_microcompact：把"删掉旧 tool 结果"包装成 API 层的 cache\_edits 指令。客户端发出去的 prompt 原封不动，服务端在已缓存的前缀上直接抠掉指定内容——字节没变，缓存不失效。
    
-   apiMicrocompact：更彻底，直接调 Anthropic 的 context\_management API（beta context-management-2025-06-27，Vertex / Bedrock 也支持），让服务端按 input\_tokens 阈值自动裁剪旧的工具调用。
...
![[笔记同步助手/images/cf1464de1688df34a3f555b33cd7740f_MD5.png]]
...
![[笔记同步助手/images/b2e8096e5aed339ac02e62d491b5ea59_MD5.png]]

我们之前有一个真实的 Task：一个 4 轮、177 step、59 分钟的会话，烧了 $77.3，其中 **83%（$64.8）全是 cache\_write**。要知道 cache\_write 的单价是 cache\_read 的 12.5 倍。通过 Langfuse 中的 step 数据，一目了然：
...
| PROTECTED\_TOOLS 的输出（Skill / Task） | 高度结构化的关键信息或会话级状态 |
| MICRO\_COMPACTION\_EXEMPT 的工具（Task / AskUserQuestion） | 保住对话流的关键节点 |
...
内容效果不满意？[点此反馈](https://feedback.notebooksyncer.com/feedback/af7bd66f_1781509007154?u=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2FBQwyvE2qIfguwKk63F3LZw&s=obsidian)